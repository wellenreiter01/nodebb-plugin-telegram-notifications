...
var 	db = require('./nodebb').db,
	...
	Telegram.addUserSettings = function(data, callback) {
        //console.log('Telegram addUserSettings');
        // create infotext for into field
        var infoText='';
                ...
                meta.settings.get('telegram-notification', function(err, plugin_settings) {
                    for (var prop in plugin.config) {
                        if (plugin_settings.hasOwnProperty(prop)) {
                            plugin.config[prop] = plugin_settings[prop];
